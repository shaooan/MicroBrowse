...
@implementation MBHostSettingsController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	MBHostSettings *hs = [MBHostSettings forHost:[[self sortedHosts] objectAtIndex:indexPath.row]];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
	if (editingStyle == UITableViewCellEditingStyleDelete) {
		if ([MBHostSettings removeSettingsForHost:[[self sortedHosts] objectAtIndex:indexPath.row]]) {
...
- (void)addHost:sender
{
	...
	UIAlertAction *okAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"OK", @"OK action") style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
		...
		if (host && ![[host text] isEqualToString:@""]) {
			MBHostSettings *hs = [[MBHostSettings alloc] initForHost:[host text] withDict:nil];
			[hs save];
			[MBHostSettings persist];
			_sortedHosts = nil;
...
{
	if (_sortedHosts == nil)
		_sortedHosts = [[NSMutableArray alloc] initWithArray:[MBHostSettings sortedHosts]];
...
{
	MBHostSettings *host = [MBHostSettings forHost:thost];
