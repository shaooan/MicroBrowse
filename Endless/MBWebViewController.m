...
#import "MBSSLCertificateViewController.h"
#import "MBURLInterceptor.h"
#import "MBWebViewController.h"
#import "MBWebViewTab.h"
#import "MBWebViewMenuController.h"
...
@implementation MBWebViewController {
	...
	MBBookmarkController *bookmarks;
}
...
- (void)encodeRestorableStateWithCoder:(NSCoder *)coder
{
	...
	for (MBWebViewTab *wvt in webViewTabs) {
		if (wvt.url == nil)
			continue;
...
#endif
		MBWebViewTab *wvt = [self addNewTabForURL:[params objectForKey:@"url"] forRestoration:YES withCompletionBlock:nil];
...
//        [self addNewTabForURL:[NSURL URLWithString:[se objectForKey:@"homepage_url"]]];
        [self addNewTabForURL:[NSURL URLWithString:@"https://www.baidu.com"]];
	}
...
- (void)adjustLayout
{
	...
	for (int i = 0; i < webViewTabs.count; i++) {
		MBWebViewTab *wvt = webViewTabs[i];
...
- (void)setCurTabIndex:(int)tab
{
	...
	for (int i = 0; i < webViewTabs.count; i++) {
		MBWebViewTab *wvt = [webViewTabs objectAtIndex:i];
...
- (MBWebViewTab *)addNewTabForURL:(NSURL *)url forRestoration:(BOOL)restoration withCompletionBlock:(void(^)(BOOL))block
{
	MBWebViewTab *wvt = [[MBWebViewTab alloc] initWithFrame:[self frameForTabIndex:webViewTabs.count] withRestorationIdentifier:(restoration ? [url absoluteString] : nil)];
...
- (void)removeTab:(NSNumber *)tabNumber andFocusTab:(NSNumber *)toFocus
{
	...
	MBWebViewTab *wvt = (MBWebViewTab *)webViewTabs[tabNumber.intValue];
...
- (void)removeAllTabs
{
	...
	for (int i = 0; i < webViewTabs.count; i++) {
		MBWebViewTab *wvt = (MBWebViewTab *)webViewTabs[i];
		[[wvt viewHolder] removeFromSuperview];
		[wvt close];
	}
...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
	...
	if (bookmarks == nil) {
		bookmarks = [[MBBookmarkController alloc] init];
...
{
	popover = [[WYPopoverController alloc] initWithContentViewController:[[MBWebViewMenuController alloc] init]];
...
- (void)settingsViewControllerDidEnd:(IASKAppSettingsViewController *)sender
{
	...
	NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
	[MBURLInterceptor setSendDNT:[userDefaults boolForKey:@"send_dnt"]];
...
- (void)showTabsWithCompletionBlock:(void(^)(BOOL))block
{
	if (showingTabs == false) {
		...
		[UIView animateWithDuration:0.25 delay:0.0 options:UIViewAnimationOptionCurveEaseOut animations:^(void) {
			for (int i = 0; i < webViewTabs.count; i++) {
				[(MBWebViewTab *)webViewTabs[i] zoomOut];
			}
		...
		[UIView animateWithDuration:0.25 delay:0.0 options:UIViewAnimationOptionCurveEaseOut animations:^(void) {
			for (int i = 0; i < webViewTabs.count; i++) {
				[(MBWebViewTab *)webViewTabs[i] zoomNormal];
			}
...
- (void)showSSLCertificate
{
	...
	MBSSLCertificateViewController *scvc = [[MBSSLCertificateViewController alloc] initWithSSLCertificate:[[self curWebViewTab] SSLCertificate]];
