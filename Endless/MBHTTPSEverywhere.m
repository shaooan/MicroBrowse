...
@implementation MBHTTPSEverywhere
...
+ (void)cacheRule:(MBHTTPSEverywhereRule *)rule forName:(NSString *)name
{
	if (!ruleCache) {
...
+ (MBHTTPSEverywhereRule *)cachedRuleForName:(NSString *)name
{
	...
	r = [[MBHTTPSEverywhereRule alloc] initWithDictionary:[[[self class] rules] objectForKey:name]];
...
+ (NSURL *)rewrittenURI:(NSURL *)URL withRules:(NSArray *)rules
{
	...
	for (MBHTTPSEverywhereRule *rule in rules) {
		if ([[MBHTTPSEverywhere disabledRules] valueForKey:[rule name]] != nil)
			continue;
...
{
	for (MBHTTPSEverywhereRule *rule in [[self class] potentiallyApplicableRulesForHost:fromHost]) {
		if ([[MBHTTPSEverywhere disabledRules] valueForKey:[rule name]] != nil)
			continue;
...
+ (void)noteInsecureRedirectionForURL:(NSURL *)URL
{
	...
	for (MBHTTPSEverywhereRule *rule in [[self class] potentiallyApplicableRulesForHost:[URL host]]) {
