...
+ (void)persistList
{
	...
	for (int i = 0; i < [_list count]; i++)
		[t addObject:((MBBookmark *)_list[i]).marshallable];
...
+ (MBBookmark *)unmarshall:(NSDictionary *)marshalled
{
...
{
	for (int i = 0; i < [[MBBookmark list] count]; i++) {
		MBBookmark *b = [MBBookmark list][i];
...
+ (UIAlertController *)addBookmarkDialogWithOkCallback:(void (^)(void))callback
{
	...
	UIAlertAction *okAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"OK", @"OK action") style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
		...
		if (url && ![[url text] isEqualToString:@""]) {
			[MBBookmark addBookmarkForURLString:[url text] withName:[name text]];
