...
#import "MBCookieController.h"
#import "MBHostSettings.h"
#import "MBHostSettingsController.h"
...
@implementation MBWebViewMenuController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if ([func isEqualToString:@"menuAddOrManageBookmarks"]) {
		if (haveURL && [MBBookmark isURLBookmarked:[[[appDelegate webViewController] curWebViewTab] url]]) {
		...
		MBHostSettings *hs = [MBHostSettings settingsOrDefaultsForHost:[[[[appDelegate webViewController] curWebViewTab] url] host]];
		if (hs && ![hs isDefault]) {
...
{
    MBWebViewTab *wvt = [[appDelegate webViewController] curWebViewTab];
	if (wvt && [wvt url])
...
{
	MBHTTPSEverywhereRuleController *herc = [[MBHTTPSEverywhereRuleController alloc] init];
...
{
	MBHostSettingsController *hsc = [[MBHostSettingsController alloc] init];
...
{
	MBBookmarkController *bc = [[MBBookmarkController alloc] init];
