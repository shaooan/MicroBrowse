...
@implementation MBWebViewTab

AppDelegate *appDelegate;

+ (MBWebViewTab *)openedWebViewTabByRandID:(NSString *)randID
{
	for (MBWebViewTab *wvt in [[appDelegate webViewController] webViewTabs]) {
...
- (BOOL)webView:(UIWebView *)__webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
	...
	if ([action isEqualToString:@"noop"]) {
		...
		if (navigationType == UIWebViewNavigationTypeLinkClicked) {
			MBWebViewTab *newtab = [[appDelegate webViewController] addNewTabForURL:nil];
			newtab.randID = param;
		...
		MBWebViewTab *wvt = [[self class] openedWebViewTabByRandID:param];
...
{
	/* reset and then let MBWebViewController animate to our actual progress */
...
- (void)setSSLCertificate:(MBSSLCertificate *)SSLCertificate
{
...
- (void)longPressMenu:(UILongPressGestureRecognizer *)sender {
	...
	UIAlertAction *saveImageAction = [UIAlertAction actionWithTitle:@"Save Image" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
		...
		[MBURLInterceptor temporarilyAllow:imgurl];
